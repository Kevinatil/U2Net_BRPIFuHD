...
def get_path():
    ...
    tra_lbl_name_list=[]
    data_dir='D:/dance/new_dance_scoring/3Dreconstruction/trainingtest1_u2net'
    tra_image_dir = 'JPEGImages'
    tra_label_dir = 'SegmentationClass'
    #tra_img_name_list = glob.glob(os.path.join(data_dir, tra_image_dir, '*'))
    tra_img_name_list = glob.glob(os.path.join(data_dir, tra_label_dir, '*'))
...
def main():
    ...
    image_ext = '.jpg'
    label_ext = '_matte.png'

    model_dir = 'D:/dance/new_dance_scoring/3Dreconstruction/trainingtest1_u2net/saved_models/' + model_name + '/'
    ...
    plt.savefig('D:/dance/new_dance_scoring/3Dreconstruction/trainingtest1_u2net/experiment/loss.png')
...
def predict():
    ...
    for i, data in tqdm(enumerate(salobj_dataloader)):
        ...
        if torch.cuda.is_available():
            net.cuda()  # GPU
        params_path='D:/dance/new_dance_scoring/3Dreconstruction/trainingtest1_u2net/saved_models/u2net/u2net.pth'
        net.load_state_dict(torch.load(params_path))
        #img = img.view(1, 3, 512, 512)
        d0, d1, d2, d3, d4, d5, d6 = net(inputs_v)

        plt.imshow(np.array((d0[0][0].cpu().detach())*255).astype('uint8'))
        plt.savefig('D:/dance/new_dance_scoring/3Dreconstruction/trainingtest1_u2net/predicts/{0}predict.png'.format(counter))
        plt.imshow(np.array((labels[0][0])*255).astype('uint8'))
        plt.savefig('D:/dance/new_dance_scoring/3Dreconstruction/trainingtest1_u2net/predicts/{0}label.png'.format(counter))
        counter+=1
...
def evaluate_trainingset(percent=1):
    ...
    for i, data in tqdm(enumerate(salobj_dataloader)):
        ...
        if counter>evalnumber:
            break
    print('training evaluation epoch ',counter,' evalnumber ',evalnumber)
