...
def get_path():
    ...
    tra_lbl_name_list = []
    data_dir='D:/dance/new_dance_scoring/3Dreconstruction/trainingtest1_3pf'
...
def get_path_test():
    ...
    tra_lbl_name_list = []
    data_dir='D:/dance/new_dance_scoring/3Dreconstruction/trainingtest1_3pf'
    tra_image_dir = 'test_img'  # 图片所在的目录名
    tra_label_dir = 'test_mask'  # 标签所在的目录名
    tra_img_name_list = glob.glob(os.path.join(data_dir, tra_image_dir, '*'))
...
def main():
    ...
    model_dir = 'D:/dance/new_dance_scoring/3Dreconstruction/trainingtest1_3pf/saved_models/' + model_name + '/'  # 保存的参数模型所在的文件夹名
...
def predict(percent=1):
    ...
    for i, data in enumerate(salobj_dataloader):
        ...
        if torch.cuda.is_available():
            net.cuda()  # GPU
        params_path='D:/dance/new_dance_scoring/3Dreconstruction/trainingtest1_3pf/saved_models/u2net/u2net.pth'
        net.load_state_dict(torch.load(params_path))

        d0, d1, d2, d3, d4, d5, d6 = net(inputs_v)

        plt.imshow(np.array((d0[0][0].cpu().detach())*255).astype('uint8'))
        plt.savefig('D:/dance/new_dance_scoring/3Dreconstruction/trainingtest1_3pf/predicts/{0}predict.png'.format(counter))
        plt.imshow(np.array((labels[0][0])*255).astype('uint8'))
        plt.savefig('D:/dance/new_dance_scoring/3Dreconstruction/trainingtest1_3pf/predicts/{0}label.png'.format(counter))
        counter+=1
...
if __name__ == '__main__':
    ...
    if trainb:
        ...
        plt.savefig('D:/dance/new_dance_scoring/3Dreconstruction/trainingtest1_3pf/experiment/loss.png')
    if predictb:
        start=time.time()
        predict(percent=0.02)
        end=time.time()
