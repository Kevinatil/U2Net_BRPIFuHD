...
def get_path():
    ...
    tra_lbl_name_list = []
    data_dir='D:/dance/new_dance_scoring/3Dreconstruction/trainingtest2_1lf'
...
def main():
    ...
    model_dir = 'D:/dance/new_dance_scoring/3Dreconstruction/trainingtest2_1lf/saved_models/' + model_name + '/'
...
def predict(percent=1):
    ...
    for i, data in enumerate(salobj_dataloader):
        ...
        if torch.cuda.is_available():
            net.cuda()
        params_path='D:/dance/new_dance_scoring/3Dreconstruction/trainingtest2_1lf/saved_models/u2net/u2net.pth'
        net.load_state_dict(torch.load(params_path))
        d0, d1, d2, d3, d4, d5, d6 = net(inputs_v)

        plt.imshow(np.array((d0[0][0].cpu().detach())*255).astype('uint8'))
        plt.savefig('D:/dance/new_dance_scoring/3Dreconstruction/trainingtest2_1lf/predicts/{0}predict.png'.format(counter))
        plt.imshow(np.array((labels[0][0])*255).astype('uint8'))
        plt.savefig('D:/dance/new_dance_scoring/3Dreconstruction/trainingtest2_1lf/predicts/{0}label.png'.format(counter))
        counter+=1
...
if __name__ == '__main__':
    ...
    if trainb:
        ...
        plt.savefig('D:/dance/new_dance_scoring/3Dreconstruction/trainingtest2_1lf/experiment/loss.png')
    if predictb:
        start=time.time()
        predict(percent=0.05)
        end=time.time()
        print('predict time: ',end-start)
    if trainingevalb:
        start=time.time()
        evaluate_trainingset(percent=0.05)
        end=time.time()
